...
/* 1.- Select the first 5 clients where the district is 1 */
SELECT client_id 
FROM bank.client
WHERE district_id = 1
LIMIT 5;
...
/* 8.-What are the unique values of k_symbol in the order table? Note: There shouldn't be a 
table name order, since order is reserved from the ORDER BY clause. You have to use 
backticks to escape the order table name. */
...
/*14.- In the card table, how many cards exist for each type? Rank the result 
starting with the most frequent type. */
...
/* 18.-In the trans table, for account_id 396, sum the amount of transactions for each 
type (VYDAJ = Outgoing, PRIJEM = Incoming). Your output should have the account_id, 
...
SELECT account_id, 
CASE `type` 
	WHEN "PRIJEM" THEN ROUND(sum(amount)) 
	WHEN "VYDAJ" THEN 0 ELSE type END 
	AS Incoming,
CASE `type` 
	WHEN "PRIJEM" THEN 0 
	WHEN "VYDAJ" THEN ROUND(sum(amount)) ELSE type END 
	AS Outgoing
FROM bank.trans
WHERE account_id = 396
GROUP BY type, account_id;
/*I didn't manage to do this one without creating a new table */
